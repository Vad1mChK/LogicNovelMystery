...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('form').addEventListener('submit', function(event) {
        ...
        if (passwordValue.length < minLength) {
            showError(`Password must be at least ${minLength} characters long`, 'errorPassword');
            return;
        } else if (!hasUpperCase) {
            showError('Password must contain at least one uppercase letter', 'errorPassword');
            return;
        } else if (!hasLowerCase) {
            showError('Password must contain at least one lowercase letter', 'errorPassword');
            return;
        } else if (!hasNumber) {
            showError('Password must contain at least one number', 'errorPassword');
            return;
        } else if (!hasSpecialChar) {
            showError('Password must contain at least one special character (!@#$%^&*)', 'errorPassword');
            return;
        }
