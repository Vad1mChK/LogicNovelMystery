...
describe('MusicController Component', () => {
    it('should call setTrack and play when currentTrack and isPlaying are set', () => {
        const store = mockStore({
            musicState: { currentTrack: 'track1.mp3', isPlaying: true, volume: 50, panning: 0 },
        });
        ...
        expect(musicManagerInstance.setTrack).toHaveBeenCalledWith('track1.mp3');
        expect(musicManagerInstance.play).toHaveBeenCalled();
    });

    it('should call pause when isPlaying is false', () => {
        const store = mockStore({
            musicState: { currentTrack: 'track1.mp3', isPlaying: false, volume: 50, panning: 0 },
        });
    ...
    it('should call setVolume with correct value', () => {
        const store = mockStore({
            musicState: { currentTrack: 'track1.mp3', volume: 80, isPlaying: true, panning: 0 },
        });
    ...
    it('should call setPanning with correct value', () => {
        const store = mockStore({
            musicState: { currentTrack: 'track1.mp3', panning: -1, isPlaying: true, volume: 50 },
        });
    ...
    it('should call stop when currentTrack is null', () => {
        const store = mockStore({
            musicState: { currentTrack: null, isPlaying: false, volume: 50, panning: 0 },
        });
